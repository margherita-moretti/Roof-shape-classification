...
Given a reference set of buildings already classified (e.g. flat vs. pitched, by visual inspection or a small hand-labelled sample), ranks every candidate variable produced by [stage 1](../1_roof_morphology) by how well it separates the two groups — used to decide which variable, and threshold, to carry forward into [stage 3](../3_ASA_index_classification).
...
2. A histogram grid (all candidate variables, saved as a single PNG — never opens a display window).

The script also checks the classification-label field for values it doesn't recognise, so a typo'd or unexpected label doesn't silently drop rows from the comparison.
...
The separability ranking is a descriptive heuristic (percentile-gap on normalized values), not a formal statistical test.
